export function mapNormalizedToRange(value: number, min: number, max: number): number {
...
export function robustModulo(value: number, modulus: number): number {
  ...
  const safeModulus = Math.abs(modulus)
  return ((value % safeModulus) + safeModulus) % safeModulus
}
  ...
  while (remainingSW > 0) {
    ...
    while (remainingSH > 0) {
      ...
      try {
        ...
        console.warn('Error in drawTiledImageSection drawImage:', error)
      }
